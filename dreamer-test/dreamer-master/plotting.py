...
PALETTES = dict(
    ...
    contrast=(
        "#377eb8", '#4daf4a', '#ff0011', '#ddaa00', '#cc44dd', '#0088aa',
...
DEFAULT_BASELINES = ['a3c_100m_proprio', 'd4pg_100m', 'planet_1e6', 'slac_3e6',]

method_mapping = {
    "dreamer": "Dreamer (5e6 steps)",
    "planet_1e6": "PlaNet (5e6 steps)",
    "d4pg_100m": "D4PG (1e8 steps)",
    "a3c_100m_proprio": "A3C (1e8 steps, proprio)",
    "slac_3e6": "Slac (3e6 steps)",
        ...
        for task in indir.iterdir():
            ...
            for method in task.iterdir():
                ...
                for seed in method.iterdir():
                    # print("seed:", seed)
                    # 收集该种子目录下所有JSONL文件
                    seed_files = list(seed.glob('**/*.jsonl'))
                    # print(f"Found {len(seed_files)} files in {seed}")
                    filenames += seed_files
    keys = set()
...
def load_baselines(patterns, prefix=False):
  runs = []
  # 使用以当前文件为基准的相对路径
  directory = (pathlib.Path(__file__).resolve().parent / 'scores').resolve()
  # print("你好！")
  # print("directory:", directory)
  # print("files:", list(directory.glob('**/*baselines.json')))
  for filename in directory.glob('**/*baselines.json'):
    # print("filename:", filename)
...
def figure(runs, methods, args):
  ...
  for ax in axes.flatten()[len(tasks) + len(args.add):]:
    ax.axis('off')
  legend(fig, method_mapping, ncol=args.legendcols, **LEGEND)
  return fig
...
def parse_args():
  ...
  parser.add_argument('--legendcols', type=int, default=6)
  ...
  parser.add_argument('--add', nargs='+', type=str, default=['auto'])
...
def process_task_name(task_name):
    """处理单个任务名称：移除dmc_前缀，将_替换为\n，并将每个单词首字母大写"""
    # 移除dmc_前缀
...
def transform_text(methods):
    """将输入文本转换为指定格式"""
    # results = []
    # for method in methods:
    #     # 使用最简单的if-else直接匹配完整方法名
    #     if method == 'dreamer':
    #         results.append("Dreamer (5e6 steps)")
    #     elif method == 'planet_1e6':
    #         results.append("PlaNet (5e6 steps)")
    #     elif method == 'd4pg_100m':
    #         results.append("D4PG (1e8 steps)")
    #     elif method == 'a3c_100m_proprio':
    #         results.append("A3C (1e8 steps, proprio)")
    #     elif method == 'slac_3e6':
    #         results.append("Slac (3e6 steps)")
    # return results
    return [method_mapping.get(method, method) for method in methods] 

def plot_final_scores_from_runs(args, output_path="bar_comparison_optimized.png"):
    runs = load_runs(args)
    baselines = load_baselines(args.baselines, args.prefix)
    # print("baselines:", baselines)
    # 只保留runs中出现的任务
    run_tasks = sorted(set(r.task for r in runs))
    all_runs = [r for r in runs + baselines if r.task in run_tasks]
    tasks = run_tasks
    # print("tasks:", tasks)
    processed_tasks = [process_task_name(task) for task in tasks]
    # print("processed_tasks:", processed_tasks)
    methods = sorted(set(r.method for r in all_runs if 'slac' not in r.method.lower())) # 跳过包含'slac'的方法
    # print("methods:", methods)
    transform_methods = transform_text(methods)
    # print("transform_methods:", transform_methods)
    returns = {method: [] for method in transform_methods}
    for task in tasks:
        for i in range(len(methods)):
            # print(i)
            relevant = [r for r in all_runs if r.task == task and r.method == methods[i]]
            if not relevant:
                returns[transform_methods[i]].append(None)
            else:
                if relevant[0].ys is not None and hasattr(relevant[0].ys, "__len__"):
                    value = relevant[0].ys[-1] if len(relevant[0].ys) > 0 else None
                else:
                    value = relevant[0].ys
                returns[transform_methods[i]].append(value)
    # 指定新的顺序
    new_order = ["Dreamer (5e6 steps)", "PlaNet (5e6 steps)", "D4PG (1e8 steps)", "A3C (1e8 steps, proprio)"]
    # 创建新字典并按新顺序插入键值对
    returns = {key: returns[key] for key in new_order}
    # print(returns)
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    colors = {
        "Dreamer (5e6 steps)": "#377eb8",
        "PlaNet (5e6 steps)": "#4daf4a",
        "D4PG (1e8 steps)": "#ff7f00",
        "A3C (1e8 steps, proprio)": "#e41a1c",
        "Slac (3e6 steps)": "#a65628"
    }
    methods = list(returns.keys())
    num_tasks = len(tasks)
    num_methods = len(methods)
    spacing = 1.2
    indices = np.arange(num_tasks) * spacing
    # bar_width = 0.2
    bar_width = 0.8 / max(num_methods, 1)
    ...
    for i, method in enumerate(methods):
        ...
        # 处理缺失值 (缺失值设为0高度)
        values = [v if v is not None else 0 for v in returns[method]]
        bars = ax.bar(indices + offset, values, bar_width, label=method, color=colors[method], edgecolor='black', alpha=0.8)
        # 为缺失值添加"N/A"标签，调整y坐标使其距X轴有距离
        for j, value in enumerate(returns[method]):
            if value is None:
              # 原bar_width为0，现设置为固定偏移量（例如20）
              ax.text(indices[j] + offset, 20, 'N/A', ha='center', va='bottom', rotation=90, fontsize=10)
    # Y轴设置
    ax.set_ylabel("Episode Return")
    all_values = [v for method_values in returns.values() for v in method_values if v is not None]
    if all_values:
        ax.set_ylim(0, max(all_values) * 1.1)
    else:
        ax.set_ylim(0, 1000)
    # 设置X轴范围
    x_min = indices[0] - bar_width * 2
    x_max = indices[-1] + bar_width * 2
    ax.set_xlim(x_min, x_max)
    # 设置X轴刻度和标签
    ax.set_xticks(indices)
    ax.set_xticklabels(processed_tasks, rotation=0, ha='center')
    # 图例放在顶部中央
    ax.legend(loc='lower center', bbox_to_anchor=(0.5, 1.02), ncol=min(4, num_methods), frameon=False)
    # 美观处理
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.grid(axis='y', linestyle='--', alpha=0.7)
    # 修改X轴显示设置
    ax.spines['bottom'].set_visible(False)  # 隐藏整个X轴
    # 在每个任务下方添加加粗短横线
    for pos in indices:
        ax.plot([pos - bar_width * 2, pos + bar_width * 2], [0, 0], color='black', linewidth=5)
    plt.tight_layout(rect=[0.01, 0, 1, 0.95])
    output_path = args.outdir / output_path
    # 保存图像
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    print(f"已保存图表到 {output_path}，共{num_tasks}个任务，{num_methods}种方法")

if __name__ == '__main__':
  args = parse_args()
  main(args)
  # plot_final_scores_from_runs(args)
